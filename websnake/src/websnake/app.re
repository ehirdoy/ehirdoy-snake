...
module Top = {
  ...
  let make _children => {
    ...
    let heartbeat _event state self => {
      ...
      | true => ReasonReact.Update {...(Model.next_state state), dir_mutex:`Released}
    ...
    let handleKeyboard event state self => {
      ...
      let new_state = {
        ...
        switch game_event {
        | `Start => {
            {...state, dir:new_dir, running: true}
          }
        | `Stop => {...state, running: false}
        | `Change_direction => {
            Model.change_direction state new_dir
          };
        | `Check_for_space => {
            switch key_pressed {
            | `Space => {
      ...
      render: fun state self => {
        ...
        <div>
          <div style=(ReactDOMRe.Style.make
                      ...
                      fontWeight::"bold"
                      ())> (ReasonReact.stringToElement message) </div>
