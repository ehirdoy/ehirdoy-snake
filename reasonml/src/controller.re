...
type game_event = [ Notty.Unescape.event | `Resize (int, int) | `End | `Refresh];
...
let start_refresh_timer () => Lwt_unix.sleep (1.0 /. 15.0) >|= (fun () => `Refresh);

let read_event (term: Term.t) :Lwt.t game_event =>
  Lwt_stream.get (Term.events term) >|= (
    fun
...
let handle_noop: event_handler = fun event state => Lwt.return state;

let handle_refresh: event_handler =
  fun event state =>
    state |> render dim::state.dim |> Term.image term >>= (
      fun () => state |> Model.next_state |> Lwt.return
    );
...
let handle_resize dim :event_handler => fun event state => Lwt.return {...state, dim};
...
let start_event_loop () => {
  ...
  let events = {event: read_event term, refresh: start_refresh_timer ()};
